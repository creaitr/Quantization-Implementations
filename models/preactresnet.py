...
def relu(inplace=False):
    """ReLU activation"""
    return qnn.QuantReLU(inplace=inplace, nbits=qcfg['bita'])
...
class PreActResNet_CIFAR(nn.Module):
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False,
        ...
        self.layer1 = self._make_layer(block, 16, layers[0], stride=1, first_block=True)
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.bn = bn(64 * block.expansion)
        self.relu = relu(inplace=True)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(64 * block.expansion, num_classes)
        self.fc = qnn.QuantLinear(64 * block.expansion, num_classes,
                                  nbits=qcfg['last_fc_bitw'], symmetric=qcfg['symmetric'])
...
model_cfgs_cifar = {
    20:  (PreActBlock, [3, 3, 3]),
    32:  (PreActBlock, [5, 5, 5]),
    44:  (PreActBlock, [7, 7, 7]),
    56:  (PreActBlock, [9, 9, 9]),
    110: (PreActBlock, [18, 18, 18]),
}
...
def set_model(cfg, qnn):
    ...
    if cfg.dataset in ['cifar10', 'cifar100']:
        ...
        model = PreActResNet_CIFAR(block, layers, num_classes)
